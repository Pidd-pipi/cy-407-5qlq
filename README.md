...
- 发布快照：发布时冻结展览文案、展品顺序与资料、主题色和导览节点，展厅与自动导览只读取最新发布版本，草稿改动不影响展出；缺少展品时发布会被拦截并说明缺哪件。
- 版本历史：后台列出版本号、发布时间和展品数，可将旧版本载入草稿，重新发布后才会替换展厅。
...
├── stores/        # artifact.ts, exhibition.ts, version.ts, annotation.ts, tour.ts
